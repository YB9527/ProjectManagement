...
export default {
  ...
  methods: {
    ...
    mydelete(index, row) {
      ...
      axios({
        url: "http://localhost:3000/admin/deletetypename",
        method: "POST",
        data: deleteData
      })
        .then(result => {
    ...
    getdepartment() {
      axios({
        url: "http://localhost:3000/projecttype",
        method: "POST"
      })
        .then(result => {
          for (var kk in result.data.data) {
            ...
            temp.Name = result.data.data[kk].typename;
    ...
    handleEdit(index, row) {
      this.$prompt("请输入类别名称", "提示", {
        ...
        .then(({ value }) => {
          ...
          userLoginData.append("typename", value);
          axios({
            url: "http://localhost:3000/admin/edittypename",
            method: "POST",
            data: userLoginData
          })
            .then(result => {
